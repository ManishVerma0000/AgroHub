...
export default function CustomerOrderPage() {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const response = await mobileOrderService.create(orderPayload);
      toast.success(`Order #${response.id.slice(-6).toUpperCase()} created successfully!`, { id: loadingToast });
      setCart([]);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-8 font-sans text-[#0f172a]">
      ...
      <div className="max-w-[1400px] mx-auto mb-8">
        <Link href={`/customers/${id}`} className="flex items-center gap-2 text-sm text-[#475569] hover:text-[#0f172a] transition-colors mb-6 font-medium">
                ...
                <div key={product.id} className="p-5 flex items-center justify-between hover:bg-[#f8fafc] transition-colors group">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl border border-[#e2e8f0] flex items-center justify-center font-bold transition-colors shadow-sm $\{
                    ...
                    <span className="font-bold min-w-[24px] text-center text-[#0f172a]">
                    ...
                    <button
                      ...
                      className="w-9 h-9 rounded-xl bg-[#16a34a] text-white flex items-center justify-center font-bold hover:bg-[#15803d] transition-colors shadow-md shadow-[#16a34a]/10"
                    >
                      +
                    </button>
                  </div>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="w-8 h-8 rounded-lg border border-[#e2e8f0] bg-white text-[#475569] flex items-center justify-center font-bold hover:bg-[#f8fafc] transition-colors"
                          >
                            -
                          </button>
                          <span className="font-bold text-[#0f172a] min-w-[20px] text-center">{item.quantity}</span>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
                            className="w-8 h-8 rounded-lg bg-[#16a34a] text-white flex items-center justify-center font-bold hover:bg-[#15803d] transition-colors shadow-sm"
                          >
                            +
                          </button>
                        </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#e2e8f0]">
            ...
            <div className="relative mb-6">
              <select
                className="w-full appearance-none pl-4 pr-10 py-3 bg-[#f8fafc] border border-[#e2e8f0] rounded-xl text-sm font-medium text-[#475569] outline-none cursor-pointer hover:border-[#cbd5e1] transition-all"
                ...
                {customer.addresses && customer.addresses.length > 0 ? (
                  customer.addresses.map((addr: any) => (
                    <option key={addr.id} value={addr.id}>
                      {addr.location} {addr.isDefault ? "(Default)" : ""}
                    </option>
                  ))
                ) : (
            ...
            {customer.addresses?.find((a: any) => a.id === selectedAddressId) && (
              <div className="p-5 bg-gradient-to-br from-[#f0f9ff] to-[#e0f2fe] rounded-2xl border border-[#bae6fd]">
                <div className="flex flex-col gap-3">
                  <h4 className="font-bold text-[#0369a1] text-[16px]">{customer.shopName || customer.ownerName}</h4>
                  <div className="flex items-start gap-2.5 text-[#0c4a6e] text-[13.5px] leading-relaxed">
                    ...
                    <span>{customer?.addresses?.find((a: any) => a.id === selectedAddressId)?.location}</span>
                  </div>
          ...
          <button
            ...
            className="w-full py-4 bg-[#16a34a] hover:bg-[#15803d] disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-2xl transition-all shadow-lg shadow-[#16a34a]/20 text-[16px] lg:mt-2"
          >
