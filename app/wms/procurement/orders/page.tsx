...
export default function PurchaseOrdersPage() {
  ...
  const mockOrders = [
    { 
      ...
      orderDate: "10 Mar\n2024", 
      expectedDelivery: "12 Mar 2024", 
      items: "2\nitems",
      plannedAmount: "15,000",
      actualAmount: "15,200",
      status: "Completed",
    },
    { 
    ...
    <div className="flex flex-col gap-6 max-w-[1400px]">
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-xl shadow-sm overflow-hidden flex flex-col">
            ...
            <tbody className="divide-y divide-[#e2e8f0]">
              {mockOrders.map((order, idx) => (
                <tr key={idx} className="hover:bg-[#f8fafc] transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-col font-bold text-[#0f172a] text-[14px]">
                      <span>{order.supplier.split('\n')[0]}</span>
                      {order.supplier.split('\n')[1] && <span>{order.supplier.split('\n')[1]}</span>}
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex flex-col text-[#64748b] font-medium">
                      <span>{order.orderDate.split('\n')[0]}</span>
                      <span>{order.orderDate.split('\n')[1]}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-[#64748b] font-medium">{order.expectedDelivery}</td>
                  <td className="px-5 py-4">
                    <div className="flex flex-col text-[#1e293b] font-semibold">
                      <span>{order.items.split('\n')[0]}</span>
                      <span className="text-[#64748b] font-normal text-xs">{order.items.split('\n')[1]}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 font-bold text-[#1e293b]">
                    ₹{order.plannedAmount}
                  </td>
                  <td className={`px-5 py-4 font-bold ${order.actualAmount === '-' ? 'text-[#16a34a]' : 'text-[#16a34a]'}`}>
                    {order.actualAmount === '-' ? '-' : `₹${order.actualAmount}`}
                  </td>
                  ...
                  <td className="px-5 py-4 text-center">
                    <div className="flex items-center justify-center gap-4">
                       ...
                       <button className="text-[#64748b] hover:text-[#1e293b] transition-colors" title="Document">
        ...
        <div className="p-4 border-t border-[#e2e8f0] flex justify-between items-center bg-white">
          <span className="text-sm text-[#475569]">Showing 1 to 2 of 2 results</span>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
            {/* Modal Header */}
            <div className="flex justify-between items-center p-6 border-b border-[#e2e8f0]">
              <h2 className="text-[20px] font-semibold text-[#0f172a]">Create Purchase Order</h2>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex flex-col gap-5">
              <div>
                <label className="block text-sm font-semibold text-[#1e293b] mb-1.5">Supplier <span className="text-[#475569] font-normal">*</span></label>
                <div className="relative">
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1e293b] mb-1.5">Order Date <span className="text-[#475569] font-normal">*</span></label>
                <div className="relative">
                  <input 
                    type="date" 
                    defaultValue="2026-03-21"
                    className="w-full appearance-none px-4 py-2 border border-[#cbd5e1] bg-white rounded-lg text-[#1e293b] text-sm outline-none focus:border-[#15803d] cursor-pointer"
                  />
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-[#1e293b] mb-1.5">Total Amount <span className="text-[#475569] font-normal">*</span></label>
                <input 
                  type="text" 
                  placeholder="₹50,000"
                  className="w-full px-4 py-2 border border-[#cbd5e1] rounded-lg text-sm text-[#0f172a] outline-none focus:border-[#15803d]" 
                />
                <p className="text-[#64748b] text-[13px] mt-1.5">You can add items after creating the PO</p>
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={3}
                  className="w-full px-4 py-3 border border-[#cbd5e1] rounded-lg text-sm text-[#0f172a] placeholder:text-[#94a3b8] outline-none focus:border-[#15803d] resize-none"
                ></textarea>
              </div>
            </div>

            {/* Modal Footer */}
            <div className="p-6 pt-2 flex justify-end gap-3 bg-white">
              <button 
                onClick={() => setShowCreateModal(false)}
              ...
              <button 
                onClick={() => setShowCreateModal(false)}
                className="px-6 py-2.5 bg-[#15803d] hover:bg-[#166534] text-white font-semibold rounded-lg transition-colors text-sm shadow-sm"
              >
                Create PO
              </button>
            </div>
